...
//
/**
 * REVISE BOTH CODES, just 1-2 LINE Changed in both regular expression and wildcard!
 */
#include <bits/stdc++.h>
 ...
 */
        ...
        if (j < p.size() && p[j] == '*') {
            //empty
            bool ans1 = solve(s, p, i, j + 1);
            //replace with previous character
            bool ans2=false;
            if(i<s.size()){ //cond doesnt need to be here 'i' doesnt need to match with 'j'.
